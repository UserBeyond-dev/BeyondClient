...
Instance.new("UICorner", CandyPop).CornerRadius = UDim.new(0, 4)local TB = Instance.new("TextButton", AT)
TB.Size = UDim2.new(1, 0, 1, 0)
TB.BackgroundTransparency = 1; TB.Text = ""; TB.ZIndex = 100

local MM = Instance.new("Frame", SG)
MM.Size = UDim2.new(0.45, 0, 0.70, 0)
...
Bar.BackgroundTransparency = 1; Bar.ZIndex = 11

local Title = Instance.new("TextLabel", Bar)
Title.Size = UDim2.new(0.6, 0, 1, 0)
Title.Text = "USERBEYOND // DEV_CONSOLE_v3.0"
Title.TextColor3 = BL; Title.TextSize = 13; Title.TextXAlignment = Enum.TextXAlignment.Left; Title.Font = Enum.Font.Code; Title.ZIndex = 11
...
Cl.Text = "X"; Cl.TextColor3 = WH; Cl.ZIndex = 12; Instance.new("UICorner", Cl)
...
Mn.Text = "—"; Mn.TextColor3 = WH; Mn.ZIndex = 12; Instance.new("UICorner", Mn)
...
local function cB(t)
	...
	b.Text = ">> " .. t .. " [ВЫКЛ]"
	b.TextColor3 = WH; b.TextSize = 12; b.Font = Enum.Font.Code; b.TextXAlignment = Enum.TextXAlignment.Left; b.ZIndex = 11; Instance.new("UICorner", b)
	return b
end
...
local bSth = cB("РЕЖИМ СКРЫТНОСТИ (Убрать из Лидеров и Скрыть Ник)")TB.TouchTap:Connect(function() AT.Visible = false; MM.Visible = true end)
...
bSp.MouseButton1Click:Connect(function()
	if St.Speed == 16 then
		St.Speed = 35; bSp.Text = ">> ФОРС СКОРОСТИ КЛИЕНТА [АКТИВЕН: 35]"
		bSp.BackgroundColor3 = Color3.fromRGB(0, 80, 150)
	else
		St.Speed = 16; bSp.Text = ">> ФОРС СКОРОСТИ КЛИЕНТА (WalkSpeed = 35)"
...
bFm.MouseButton1Click:Connect(function()
	St.AutoFarm = not St.AutoFarm; if St.AutoFarm then updateRemoteCache() end
...
bSth.MouseButton1Click:Connect(function()
	...
	if not St.Stealth then
		local pList = game:GetService("CoreGui"):FindFirstChild("PlayerList") or L.PlayerGui:FindFirstChild("PlayerList") if pList then pList.Enabled = true end
		local leader = L:FindFirstChild("leaderstat") or L:FindFirstChild("leaderstats") if leader then for _, stat in ipairs(leader:GetChildren()) do if origStats[stat.Name] then stat.Value = origStats[stat.Name] end end end
		table.clear(origStats)
	end
end)
